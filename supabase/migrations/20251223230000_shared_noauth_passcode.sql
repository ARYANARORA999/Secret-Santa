...
-- 2) Utility: constant-time-ish comparison isn't necessary here; we store SHA-256 hex.
CREATE OR REPLACE FUNCTION public.sha256_hex(input text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
-- 5) Join event / upsert participant (by display_name)
CREATE OR REPLACE FUNCTION public.ss_upsert_participant(p_event_id uuid, p_passcode text, p_display_name text)
RETURNS public.participants
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- No auth.users, so we generate a stable-ish user_id per display name.
  -- (Not perfect, but fine for a party app.)
  -- We maintain UNIQUE(event_id, user_id) from the original schema.
  INSERT INTO public.participants (event_id, user_id, display_name)
  VALUES (p_event_id, gen_random_uuid(), trim(p_display_name))
  RETURNING * INTO p;
...
-- 10) RLS updates: allow public reads, restrict writes to RPC only.
-- Enable RLS stays on.
